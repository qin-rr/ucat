...
class HyperX(torch.utils.data.Dataset):
    ...
    def __init__(self, data, gt, **hyperparams):
        ...
        self.datapad = np.pad(self.data, ((p, p), (p, p), (0, 0)), mode='constant')
        self.labelpad = np.pad(self.label, ((p, p), (p, p)), mode='constant')
    ...
    def __getitem__(self, i):
        ...
        p = self.patch_size // 2
        data = self.datapad[x1 + p : x2 + p, y1 + p : y2 + p]
        label = self.labelpad[x1 + p : x2 + p, y1 + p : y2 + p]
